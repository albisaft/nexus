...
//       if (kingzone[i] == 1) Attack_Bauer += KSafety;
    ...
    for (int i=21; i < 99; i++) {
     ...
     if (zugzone_ich[i] > zugzone_du[i] && (zugzone_du[i] > 0 || feld[i] != 0)) {Kontrolle_ich += 1; if ((zugzone_ich[i+1] > zugzone_du[i+1]) || (zugzone_ich[i-1] > zugzone_du[i-1] ) || (zugzone_ich[i+10] > zugzone_du[i+10] ) || (zugzone_ich[i-10] > zugzone_du[i-10] ) || (zugzone_ich[i-11] > zugzone_du[i-11] )|| (zugzone_ich[i-9] > zugzone_du[i-9] )|| (zugzone_ich[i+9] > zugzone_du[i+9] )|| (zugzone_ich[i+11] > zugzone_du[i+11]))  Koordination_ich += 1; if ((zugzone_ich[i+1] == zugzone_du[i+1]) || (zugzone_ich[i-1] == zugzone_du[i-1] ) || (zugzone_ich[i+10] == zugzone_du[i+10] ) || (zugzone_ich[i-10] == zugzone_du[i-10] )|| (zugzone_ich[i+11] == zugzone_du[i+11] )|| (zugzone_ich[i+9] == zugzone_du[i+9] )|| (zugzone_ich[i-9] == zugzone_du[i-9] )|| (zugzone_ich[i-11] == zugzone_du[i-11] )) Koordination_ich += 0.5; }
        if (zugzone_du[i] > zugzone_ich[i] && (zugzone_ich[i] > 0 || feld[i] != 0)) {Kontrolle_du += 1; if ((zugzone_du[i+1] > zugzone_ich[i+1]) || (zugzone_du[i-1] > zugzone_ich[i-1] ) || (zugzone_du[i+10] >  zugzone_ich[i+10] ) || (zugzone_du[i-10] > zugzone_ich[i-10] || (zugzone_du[i+11] >  zugzone_ich[i+11] )|| (zugzone_du[i+9] >  zugzone_ich[i+9] )|| (zugzone_du[i-11] >  zugzone_ich[i-11] )|| (zugzone_du[i-9] >  zugzone_ich[i-9] ))) Koordination_du += 1; if ((zugzone_du[i+1] == zugzone_ich[i+1]) || (zugzone_du[i-1] == zugzone_ich[i-1] ) || (zugzone_du[i+10] == zugzone_ich[i+10] ) || (zugzone_du[i-10] == zugzone_ich[i-10]|| (zugzone_du[i-11] == zugzone_ich[i-11] ) || (zugzone_du[i-9] == zugzone_ich[i-9] ) || (zugzone_du[i+9] == zugzone_ich[i+9] ) || (zugzone_du[i+11] == zugzone_ich[i+11] )  )) Koordination_du += 0.5; }
